...
describe("InspectImageTool", () => {
	...
	it("reports original dimensions without a resize claim when the image is sent unchanged", async () => {
		...
		const metadata = "Image metadata: original dimensions 1x1; sent at 1x1 for analysis.";
		expect(result.content).toEqual([{ type: "text", text: `${metadata}\n\nDetected text: Settings` }]);
		const unchangedText = result.content.find(block => block.type === "text")?.text ?? "";
		expect(unchangedText).not.toMatch(/\b(?:upscaled|downscaled)\b/);
		...
		expect(contentParts[1]).toEqual({ type: "text", text: metadata });
		expect(contentParts[2]).toEqual({ type: "text", text: "Extract visible UI labels." });
	});

	it("discloses original and sent dimensions when provider analysis resizes the image", async () => {
		const stub = createCompleteSimpleSuccessStub("The source image is 1x1.");
		...
		const result = await tool.execute("call-resized-dimensions", {
			path: imagePath,
			question: "What are the source image dimensions?",
		});

		const metadata = "Image metadata: original dimensions 1x1; upscaled to 200x200 for analysis.";
		expect(result.content).toEqual([{ type: "text", text: `${metadata}\n\nThe source image is 1x1.` }]);
		expect(result.details?.dimensions).toEqual({
			original: { width: 1, height: 1 },
			sent: { width: 200, height: 200 },
			resizeApplied: true,
		});
		const request = stub.calls[0]?.[1] as { messages?: Array<{ content?: unknown }> } | undefined;
		...
		expect(contentParts[0]?.type).toBe("image");
		expect(contentParts[1]).toEqual({ type: "text", text: metadata });
		expect(contentParts[2]).toEqual({ type: "text", text: "What are the source image dimensions?" });
	});
	...
	it("sends question text unchanged", async () => {
		...
		const content = userMessage?.content;
		const contentParts = (Array.isArray(content) ? content : []) as Array<{ type: string; text?: string }>;
		expect(contentParts[2]).toEqual({ type: "text", text: "What warning is shown?" });
	});
	...
	it("skips the timeout guard when inspect_image.timeoutMs is zero", async () => {
		...
		expect(result.content).toEqual([
			{
				type: "text",
				text: "Image metadata: original dimensions 1x1; sent at 1x1 for analysis.\n\nTimeout disabled path",
			},
		]);
		expect(stub.calls).toHaveLength(1);
