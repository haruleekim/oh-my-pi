...
import { parseImageMetadata, prompt, readImageMetadata } from "@oh-my-pi/pi-utils";
...
const RESIZED_DIMENSION_NOTE_PATTERN = /\boriginal (\d+)x(\d+), displayed at (\d+)x(\d+)\b/;
...
export interface InspectImageDimensions {
	original: PixelDimensions;
	sent: PixelDimensions;
	resizeApplied: boolean;
}

function inspectImageDimensions(
	imageInput: LoadedImageInput,
	sourceDimensions?: PixelDimensions,
): InspectImageDimensions {
	const sentMetadata = parseImageMetadata(Buffer.from(imageInput.data, "base64"));
	if (!sentMetadata?.width || !sentMetadata.height) {
		throw new ToolError(`Could not determine pixel dimensions for ${imageInput.resolvedPath}.`);
	}

	let original = sourceDimensions ?? { width: sentMetadata.width, height: sentMetadata.height };
	if (!sourceDimensions) {
		const resizeMatch = imageInput.dimensionNote?.match(RESIZED_DIMENSION_NOTE_PATTERN);
		if (resizeMatch) {
			const [, width, height] = resizeMatch;
			if (width && height) {
				original = { width: Number.parseInt(width, 10), height: Number.parseInt(height, 10) };
			}
		}
	}

	const sent = { width: sentMetadata.width, height: sentMetadata.height };
	return {
		original,
		sent,
		resizeApplied: original.width !== sent.width || original.height !== sent.height,
	};
}

function formatImageMetadata(dimensions: InspectImageDimensions): string {
	const original = `${dimensions.original.width}x${dimensions.original.height}`;
	const sent = `${dimensions.sent.width}x${dimensions.sent.height}`;
	if (!dimensions.resizeApplied) {
		return `Image metadata: original dimensions ${original}; sent at ${sent} for analysis.`;
	}

	const originalPixels = dimensions.original.width * dimensions.original.height;
	const sentPixels = dimensions.sent.width * dimensions.sent.height;
	const operation = sentPixels >= originalPixels ? "upscaled" : "downscaled";
	return `Image metadata: original dimensions ${original}; ${operation} to ${sent} for analysis.`;
}
...
export interface InspectImageToolDetails {
	...
	imagePath: string;
	mimeType: string;
	dimensions: InspectImageDimensions;
	usage: Usage;
}
...
export class InspectImageTool implements AgentTool<typeof inspectImageSchema, InspectImageToolDetails> {
	...
	async execute(
		...
		const attachments = attachmentReference ? (this.session.getImageAttachments?.() ?? []) : [];
		let sourceDimensions: PixelDimensions | undefined;
		if (attachmentReference) {
			const attachment = attachments[attachmentReference.index - 1];
			const metadata = attachment ? parseImageMetadata(Buffer.from(attachment.image.data, "base64")) : undefined;
			if (metadata?.width && metadata.height) {
				sourceDimensions = { width: metadata.width, height: metadata.height };
			}
		} else if (imageTarget && !isSvgImage) {
			const metadata = await readImageMetadata(imageTarget.path);
			if (metadata?.width && metadata.height) {
				sourceDimensions = { width: metadata.width, height: metadata.height };
			}
		}
		...
		const dimensions = inspectImageDimensions(imageInput, sourceDimensions);
		const imageMetadata = formatImageMetadata(dimensions);
		...
		return {
			content: [{ type: "text", text: `${imageMetadata}\n\n${text}` }],
			details: {
